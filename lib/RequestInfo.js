...
var KeyValueEditItem = React.createClass({
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return this.state.checked !== nextState.checked ? true : false;
  },
  ...
  render() {
    ...
    return (
      <Row className="editor-item">
        <Col xs={1}><FaIconButton className="move-bar" faIconName="arrows" {...dragProps}/></Col>
        <Col xs={8} className="editor-item-content" {...dragProps}>
          <Input type="text" placeholder="Key" onChange={this.onKeyChange}/>
        ...
        <Col xs={1}><FaIconButton faIconName="remove" onClick={this.onRemove}/></Col>
      </Row>
    );
  }
});

var KeyValueNewItem = React.createClass({
  propTypes: {
  ...
  render() {
    let onActive = this.props.onActive;
    return (
      <Row className="editor-item">
        <Col xsOffset={1} xs={8} className="editor-item-content">
          <Input type="text" placeholder="Key" onFocus={onActive}/>
          <Input type="text" placeholder="Value" onFocus={onActive}/>
        </Col>
        <Col xs={1}><FaIconButton faIconName="edit" onClick={onActive}/></Col>
      </Row>
    );
  }
});
